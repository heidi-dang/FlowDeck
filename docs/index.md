...
- **13 commands** — slash-command entry points for planning, execution, verification, support, memory recall, and learning (`/fd-task`, `/fd-execute`, `/fd-verify`, `/fd-review`, `/fd-checkpoint`, `/fd-resume`, `/fd-status`, `/fd-done`, `/fd-recall`, `/fd-learning`, `/fd-memory`, `/fd-learn`, `/fd-learn-from-session`).
