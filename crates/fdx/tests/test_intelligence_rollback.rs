...
#[test]
fn test_failed_refresh_rollback() {
    ...
    let gen_before: u64 = db.get_metadata("generation").unwrap().unwrap().parse().unwrap();
    let files_before: i64 = db.conn.query_row("SELECT count(*) FROM files", [], |r| r.get(0)).unwrap();
    ...
    let gen_after: u64 = db2.get_metadata("generation").unwrap().unwrap().parse().unwrap();
    assert_eq!(gen_before, gen_after, "Generation should not advance on failed refresh");
    
    let files_after: i64 = db2.conn.query_row("SELECT count(*) FROM files", [], |r| r.get(0)).unwrap();
