...
use fdx::intelligence::model::{IndexedFile, GraphNode, GraphEdge};
use fdx::protocol::{NodeKind, EdgeKind, EvidenceStrength, EvidenceProviderKind};
...
fn test_replace_file_evidence() {
    ...
    let node_count: i32 = db.conn.query_row("SELECT count(*) FROM nodes WHERE stable_id = 'node_b'", [], |r| r.get(0)).unwrap();
    assert_eq!(node_count, 0);
    
    // node_x should still be there
    let node_x_count: i32 = db.conn.query_row("SELECT count(*) FROM nodes WHERE stable_id = 'node_x'", [], |r| r.get(0)).unwrap();
    assert_eq!(node_x_count, 1);
    
    // edge_a_x should be gone (cascade from node_b)
    let edge1_count: i32 = db.conn.query_row("SELECT count(*) FROM edges WHERE stable_id = 'edge_a_x'", [], |r| r.get(0)).unwrap();
    assert_eq!(edge1_count, 0);
    
    // edge_x_b should be gone (cascade from node_b as to_node)
    let edge2_count: i32 = db.conn.query_row("SELECT count(*) FROM edges WHERE stable_id = 'edge_x_b'", [], |r| r.get(0)).unwrap();
