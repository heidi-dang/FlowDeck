...
export interface PerformanceContext { taskClass?: string; complexityBand?: "low" | "medium" | "high"; riskBand?: "low" | "medium" | "high" }
export interface PerformanceProfile { agentId: string; capability: string; context?: PerformanceContext; sampleCount: number; eligible: boolean; status: "eligible" | "insufficient_data"; components: { success: number; verification: number; efficiency: number; latency: number; retries: number; quality: number; regression: number; integration: number }; score: number; policyVersion: string }

export function scorePerformance(observations: readonly PerformanceObservation[], agentId: string, capability: string, minimumSamples = 3, context: PerformanceContext = {}): PerformanceProfile {
  const rows = observations.filter(o => o.agentId === agentId && o.capability === capability && (!context.taskClass || o.taskClass === context.taskClass) && (!context.complexityBand || o.complexityBand === context.complexityBand) && (!context.riskBand || o.riskBand === context.riskBand))
  ...
  return { agentId, capability, ...(Object.keys(context).length ? { context } : {}), sampleCount: n, eligible: n >= minimumSamples, status: n >= minimumSamples ? "eligible" : "insufficient_data", components, score: Math.max(0, Math.min(100, score)), policyVersion: PERFORMANCE_POLICY_VERSION }
}
