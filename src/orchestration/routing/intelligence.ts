...
export function routeTask(input: TaskIntelligenceInput): RoutingDecision {
  ...
  const decision = { routingDecisionId: `route_${fingerprint({ input, assessment, selected, ws, delegations }).slice(0, 24)}`, runId: input.runId, decisionVersion: 1, sourceSha: input.sourceSha, routingMode: "enforceable" as const, assessment, strategy: selected.strategy, delegate: delegations.length > 0, delegations, workstreams: ws, budgetRecommendation, modelRecommendation: "advisory-only: preserve configured model", rationale: selected.rationale, rejectedAlternatives: selected.rejected, policyVersion: ROUTING_POLICY_VERSION, createdAt: new Date(0).toISOString(), finalized: true as const }
