...
export class SqliteContractRepoAdapter {
  ...
  async getFamily(id: string): Promise<FamilyData | undefined> {
    const r = this.db.query("SELECT * FROM contract_families WHERE family_id=?").get(id) as any; if(!r)return undefined
  ...
  async listFamilies(): Promise<FamilyData[]> {
    return (this.db.query("SELECT * FROM contract_families ORDER BY name").all() as any[]).map(r => ({ id: r.family_id, name: r.name, description: r.description, createdBy: r.created_by, createdAt: new Date(r.created_at) }))
  }
...
export class SqliteVerificationRepoAdapter {
  ...
  async listResultsByRun(runId: string): Promise<VerResultData[]> {
    return (this.db.query("SELECT * FROM verification_results WHERE run_id=? ORDER BY started_at").all(runId) as any[]).map(r => ({ id: r.id, runId: r.run_id, status: r.status, createdAt: new Date(r.started_at) }))
  }
...
export class SqliteEvidenceRepoAdapter {
  ...
  async listEvidenceByRun(runId: string): Promise<EvData[]> {
    return (this.db.query("SELECT * FROM evidence WHERE run_id=? ORDER BY created_at").all(runId) as any[]).map(r => ({ id: r.id, runId: r.run_id, sha: r.sha, content: r.content_hash, contentType: r.evidence_type, criterionIds: [], status: 'current', createdAt: new Date(r.created_at) }))
  }
  async listEvidenceByCriterion(_criterionId: string): Promise<EvData[]> { return [] }
  async listEvidenceBySha(sha: string): Promise<EvData[]> {
    return (this.db.query("SELECT * FROM evidence WHERE sha=? ORDER BY created_at").all(sha) as any[]).map(r => ({ id: r.id, runId: r.run_id, sha: r.sha, content: r.content_hash, contentType: r.evidence_type, criterionIds: [], status: 'current', createdAt: new Date(r.created_at) }))
  }
  ...
  async listLinksByEvidence(evidenceId: string): Promise<EvLinkData[]> {
    return (this.db.query("SELECT * FROM run_criterion_evidence WHERE evidence_id=?").all(evidenceId) as any[]).map(r => ({ evidenceId: r.evidence_id, criterionId: r.run_acceptance_criterion_id, relationship: r.relationship }))
  }
}
...
export class SqliteApprovalRepoAdapter {
  ...
  async getRequest(id: string): Promise<AppReqData | undefined> {
    const r = this.db.query("SELECT id,run_id,approved_by as requester,override_type as status,created_at FROM completion_overrides WHERE id=?").get(id) as any
  ...
  async listRequestsByRun(taskRunId: string): Promise<AppReqData[]> {
    return (this.db.query("SELECT id,run_id,approved_by as requester,override_type as status,created_at FROM completion_overrides WHERE run_id=?").all(taskRunId) as any[]).map(r => ({ id:r.id, runId:r.run_id, requester:r.requester, status:r.status, createdAt:new Date(r.created_at) }))
  }
...
export class SqliteOverrideRepoAdapter {
  ...
  async getRequest(id: string): Promise<OvReqData | undefined> {
    const r = this.db.query("SELECT id,run_id,override_type,created_at FROM completion_overrides WHERE id=?").get(id) as any
  ...
  async listRequestsByRun(taskRunId: string): Promise<OvReqData[]> {
    return (this.db.query("SELECT id,run_id,override_type,created_at FROM completion_overrides WHERE run_id=?").all(taskRunId) as any[]).map(r => ({ id:r.id, runId:r.run_id, overrideType:r.override_type, status:'approved', version:1, createdAt:new Date(r.created_at) }))
  }
...
export class SqliteCompletionRepoAdapter {
  ...
  async getLatestEvaluation(v: string): Promise<CompEvalData | undefined> {
    const r = this.db.query("SELECT id,run_id,status,started_at FROM verification_results WHERE run_id=? ORDER BY started_at DESC LIMIT 1").get(v) as any
  ...
  async listEvaluations(v: string): Promise<CompEvalData[]> {
    return (this.db.query("SELECT id,run_id,status,started_at FROM verification_results WHERE run_id=? ORDER BY started_at").all(v) as any[]).map(r => ({ id:r.id, contractVersionId:r.run_id, status:r.status, details:'', createdAt:new Date(r.started_at) }))
  }
  ...
  async getDecision(id: string): Promise<CompDecData | undefined> {
    const r = this.db.query("SELECT * FROM completion_decisions WHERE id=?").get(id) as any; if(!r)return undefined
  ...
  async getLatestDecisionByRun(taskRunId: string): Promise<CompDecData | undefined> {
    const r = this.db.query("SELECT * FROM completion_decisions WHERE run_id=? ORDER BY decided_at DESC LIMIT 1").get(taskRunId) as any
  ...
  async listDecisionsByRun(taskRunId: string): Promise<CompDecData[]> {
    return (this.db.query("SELECT * FROM completion_decisions WHERE run_id=? ORDER BY decided_at DESC").all(taskRunId) as any[]).map(r => ({ id:r.id, runId:r.run_id, decision:r.decision, sha:r.sha, details:r.checks, createdAt:new Date(r.decided_at) }))
  }
