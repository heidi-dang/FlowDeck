...
describe("Schema validation", () => {
  ...
  it("passes on correctly migrated database", () => {
    ...
    expect(r.details.fk_violations).toHaveLength(0)
    expect(r.details.missing_tables).toHaveLength(0)
    expect(r.details.missing_triggers).toHaveLength(0)
  ...
  it("fails on empty database", () => {
    ...
    expect(r.details.missing_tables.length).toBeGreaterThan(0)
  })
})
