...
// bun:sqlite adaptation — matches production transaction-manager.ts semantics.
...
function clean() { closeAll(); for (const f of [DB,DB+'-wal',DB+'-shm',DB+'-lock',DB+'-lock-wal',DB+'-lock-shm']) { try { unlinkSync(f) } catch {} } }
const conns = new Map();
...
// ── Transaction manager mirroring production transaction-manager.ts ──
...
// 6. Custom thenable (dynamic property name avoids static detection)
thr(() => tx.write(() => {
