...
export class FdxRuntime {
  ...
  workspaceId(workspace: string): string { return createHash("sha256").update(resolve(workspace)).digest("hex").slice(0, 32) }
  ...
  private write(entries: Entry[]): void { const bounded = entries.sort((a, b) => b.updatedAt - a.updatedAt).slice(0, this.maxEntries); const text = JSON.stringify(bounded); if (Buffer.byteLength(text) > this.maxBytes) throw new Error("FDX_CACHE_LIMIT"); mkdirSync(dirname(this.options.cacheFile), { recursive: true }); writeFileSync(this.options.cacheFile, text, "utf8") }
}
