...
describe("SqliteUnitOfWork Atomicity", () => {
  ...
  it("commits multiple operations atomically on success", async () => {
    await unitOfWork.execute((_ctx) => {
      db.query("INSERT INTO runs (id, status) VALUES (?, ?)").run("r1", "running");
      db.query("INSERT INTO events (id, type) VALUES (?, ?)").run("e1", "run.started");
    });

    const run = db.query("SELECT * FROM runs WHERE id = ?").get("r1") as any;
    const event = db.query("SELECT * FROM events WHERE id = ?").get("e1") as any;
  ...
  it("rolls back all operations if any statement throws an error", async () => {
    try {
      await unitOfWork.execute((_ctx) => {
        db.query("INSERT INTO runs (id, status) VALUES (?, ?)").run("r2", "queued");
        db.query("INSERT INTO events (id, type) VALUES (?, ?)").run("e2", "run.queued");
    ...
    const run = db.query("SELECT * FROM runs WHERE id = ?").get("r2");
    const event = db.query("SELECT * FROM events WHERE id = ?").get("e2");
