...
`
  ...
  describe("getFullModeSteps & Rust Command Assembly", () => {
    it("returns 10 standard steps with native contract freshness first when no Rust changes are detected", () => {
      const steps = getFullModeSteps(false, false)
      expect(steps).toHaveLength(10)
      expect(steps[0]).toEqual({ name: "Native Contract Freshness", cmd: "npm run check:fdx-vci-contract" })
