...
describe("Heidi Active Coordinator & Ancestry Acceptance Regressions", () => {
  it("executes complete active coordinator lifecycle: 4 children, root depth 0, child depth 1, incremental integration before last child completion", async () => {
    ...
    const rec = coord.reconcileChildren(
      children.map(c => ({
        childId: c.id,
        parentSessionId: sessionId,
        specialist: c.specialist,
    ...
    coord.recordChildLifecycleEvent({
      ...
      snapshot: {
        ...
        createdAt: tChildBCompleted - 50,
        finishedAt: tChildBCompleted,
        lastActivityAt: tChildBCompleted,
      },
    })
